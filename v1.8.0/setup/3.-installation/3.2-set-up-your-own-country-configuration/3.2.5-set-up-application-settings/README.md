...
These are your registration time periods in days and fees for **birth**, explained in more detail [here](../../../../product-specifications/core-functions/5.-print-certificate.md). PRINT\_IN\_ADVANCE allows you to print certificates in advance for collection. You can disable this by setting it to false, if you operationally want to enforce an ID check from a recipient before printing a copy.

The FEE object sets default fees for certification as a fallback if you havent configured individual fees for specific certificate templates. Explained in section [Configure certificate templates](../3.2.6-configure-certificate-templates.md)
...
```typescript
backgroundImage data:image/jpg;base64,${fs       .readFileSync(join(__dirname, 'login-bg.jpg'))       .toString('base64')},
imageFit: 'FILL'
```
