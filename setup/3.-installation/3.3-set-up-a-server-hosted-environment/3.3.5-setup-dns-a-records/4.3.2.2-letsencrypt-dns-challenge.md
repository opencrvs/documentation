# 4.3.2.2 LetsEncrypt DNS challenge

If you are provisioning a qa, staging or production environment behind a VPN and wish to use LetsEncrypt there are 2 options depending on your DNS provider. &#x20;
...
Traefik supports APIs for the following providers:
...
If you are not using one of Traefik's supported DNS providers, then you can manually create the LetsEncrypt static files .crt and .key using the certbot tool.
...
At this point you need to go to control panel of your DNS server and create the TXT record for the domain as instructed.

Once the process succeeds, it should write 2 certificate files .crt and .key to your local machine.

3. You must place these 2 files on your server where Traefik Docker container can access them in a shared volume. This process is identical to the next step: [Static TLS certificates](4.3.2.3-static-tls-certificates.md).
