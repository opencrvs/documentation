...
If the backup was taken with `minio_backup_type: differential`, the encrypted backup file does not contain attachments. Copy the `minio` folder onto the backup server as well:
...
export MINIO_BACKUP_TYPE=<enter dump or differential, matching the minio_backup_type used when the backup was taken> \
```
...
If `MINIO_BACKUP_TYPE` is `differential`, the restore script copies attachments directly from the backup server, so it also needs the backup server details:

```
cd / && bash /opt/opencrvs/infrastructure/backups/restore.sh --replicas=$REPLICAS --label=$LABEL --ssh_user=backup --ssh_host=$SSH_HOST --ssh_port=$SSH_PORT --remote_dir=/home/backup/backups >> /var/log/opencrvs-restore.log 2>&1
```
