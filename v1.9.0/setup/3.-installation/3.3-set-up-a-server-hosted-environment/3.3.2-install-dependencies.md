...
The optional **"minio\_backup\_type"** variable controls how MinIO attachments (images and documents uploaded with records) are backed up:

* `dump` (default): a full archive of all attachments is created every night and included in the encrypted backup file.
* `differential`: only new and changed attachments are copied to the backup server with rsync. Use this if you store a large volume of attachments and a full dump takes too long.

```
minio_backup_type: differential
```

{% hint style="warning" %}
If you change `minio_backup_type`, set the **same value in both production.yml and staging.yml** and re-provision both environments. Production uses it to create the backup and staging uses it to restore the backup. If the values differ, the nightly restore on staging will fail.
{% endhint %}

Read [Backup & Restore](4.3.7-backup-and-restore/) to understand how the two options differ before choosing one.
