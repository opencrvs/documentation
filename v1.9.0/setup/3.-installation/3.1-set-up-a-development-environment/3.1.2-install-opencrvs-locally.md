...
6. Now that OpenCRVS is running, the opencrvs-core databases must be seeded with location (administrative structure), test offices and test user data.  Inside the opencrvs-core window (bottom-right), run the following seeding command:

```
yarn seed:dev
```

The output of succesful seeding looks like this:
...
* You have successfully installed OpenCRVS! 🎉
* Proceed to login using the details in the [section 3.14 Log in to OpenCRVS](3.1.4-log-in-to-opencrvs-locally.md).
* To stop OpenCRVS running in the installer, type **ctrl+c** in each window
