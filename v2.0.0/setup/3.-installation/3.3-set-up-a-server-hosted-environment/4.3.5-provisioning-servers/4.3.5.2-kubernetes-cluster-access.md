...
## Configure your local kubectl to work with remote cluster
...
3. If you already have `~/.kube/config` on your laptop run following command to append new cluster to your existing configuration:
    ...
    ```
    {% hint style="warning" %}
    Sometime cluster with the same context may already exist in your local configuration file, you may get error:
    ```
    error: You must be logged in to the server (Unauthorized)
    ```
    In that case backup kubernetes config file located on your laptop at `~/.kube/config` and remove it. Run `mv /tmp/kubeconfig ~/.kube/config` after.
    {% endhint %}
